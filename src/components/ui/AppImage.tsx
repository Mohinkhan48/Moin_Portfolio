...
const AppImage = memo(function AppImage({
    ...
    if (hasError || !imageSrc) {
        return (
            <div
                className={`flex items-center justify-center bg-terminal-bg border border-phosphor/20 ${className}`}
                style={{ width: fill ? '100%' : width, height: fill ? '100%' : height }}
            >
                <div className="text-phosphor/40 flex flex-col items-center gap-2">
                    <span className="text-4xl">▧</span>
                    <span className="text-[10px] font-mono tracking-tighter opacity-50 uppercase">
